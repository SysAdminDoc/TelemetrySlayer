...
- [ ] P1 - Refresh current Microsoft Edge and Windows policy coverage
  ...
  Acceptance: Edge/WebView2 scan and apply use current Microsoft policy names, obsolete policies are marked legacy, and README lists all covered Edge policy values.
  Complexity: M

- [ ] P1 - Add durable transcript, crash log, and support bundle
  Why: Current logs live only in the WPF text box, so failed applies cannot be diagnosed after exit.
  Evidence: `TelemetrySlayer.ps1:709`, `TelemetrySlayer.ps1:1070`; Sophia Script transcript logging; Win11Debloat issue logs.
  Touches: `TelemetrySlayer.ps1`
  Acceptance: Each run writes `%ProgramData%\TelemetrySlayer\Logs\<timestamp>.log`, unhandled worker exceptions are recorded, and the GUI has an "Open Log Folder" action.
  Complexity: M

- [ ] P1 - Ship a silent RMM mode with config presets
  Why: README says silent mode can be adapted, but MSP/RMM use is a core persona and competitors support config-driven automation.
  Evidence: `README.md:164`; ChrisTitusTech/winutil JSON automation; Raphire/Win11Debloat custom mode.
  Touches: `TelemetrySlayer.ps1`, `README.md`
...
- [ ] P1 - Make the action catalog the runtime source of truth
  Why: The catalog is testable, but scan/apply/undo still maintain separate hard-coded blocks that can drift from the tested behavior.
  Evidence: `TelemetrySlayer.ps1:111`, `TelemetrySlayer.ps1:299`, `TelemetrySlayer.ps1:862`, `TelemetrySlayer.ps1:1671`; Raphire/Win11Debloat release 2026.06.24; privacy.sexy transparent script model.
  Touches: `TelemetrySlayer.ps1`, `tests/TelemetrySlayer.Tests.ps1`, `tests/TelemetrySlayer.Static.Tests.ps1`
  Acceptance: GUI scan/apply/verify/undo enumerate selected actions from `Get-TelemetrySlayerActionCatalog`, real dispatchers execute host operations, and Pester mocks the same dispatchers used by the GUI.
  Complexity: L

- [ ] P1 - Add a persistent post-apply verification ledger
  Why: Apply logs commands and rescans, but it does not persist per-operation expected/actual results or make partial failures machine-readable.
  Evidence: `TelemetrySlayer.ps1:1919`, `TelemetrySlayer.ps1:1928`, `tests/TelemetrySlayer.Tests.ps1`; Privatezilla pass/fail privacy checks; privacy.sexy auditable generated scripts.
  Touches: `TelemetrySlayer.ps1`, `tests/TelemetrySlayer.Tests.ps1`
  Acceptance: Each apply writes `%ProgramData%\TelemetrySlayer\Runs\<timestamp>\results.json` with Test/Apply/Verify status per action, the GUI summary separates applied/skipped/failed/needs-reboot, and failed required actions are visible to future silent mode exit handling.
  Complexity: M

- [ ] P1 - Detect domain and MDM policy ownership before local writes
  Why: Local registry writes can be overridden by domain GPO or MDM policy, creating false "OFF" status unless managed ownership is surfaced.
  Evidence: `TelemetrySlayer.ps1:1546`, `TelemetrySlayer.ps1:1921`; Raphire/Win11Debloat release 2026.06.24 GPO override warning; Microsoft Policy CSP docs.
  Touches: `TelemetrySlayer.ps1`, `README.md`
  Acceptance: Scan detects domain join, MDM enrollment, and conflicting managed policy where feasible, marks affected toggles as `Managed`, and Apply warns or skips unless a future force option is explicit.
  Complexity: M

- [ ] P1 - Add a signed and checksummed verify-before-run path
  Why: README currently promotes `irm | iex`, while Microsoft flags `Invoke-Expression` as a last-resort pattern for arbitrary command strings.
  Evidence: `README.md:18`; Microsoft `AvoidUsingInvokeExpression`; existing distribution roadmap for signed releases.
  Touches: `README.md`, release packaging docs/scripts when added
  Acceptance: README documents download, SHA256 verification, Authenticode signature check, and run commands for the raw PS1 or ZIP; `irm | iex` is retained only as an advanced/unattended shortcut with a clear trust warning.
  Complexity: S
...
- [ ] P2 - Add accessibility and high-contrast pass for the WPF GUI
  ...
  Evidence: `TelemetrySlayer.ps1:29`, `TelemetrySlayer.ps1:95`; Win11Debloat issue 602 shows Windows 10 UI rendering differences.
  Touches: `TelemetrySlayer.ps1`, `screenshot.png`
  Acceptance: Status indicators include text plus accessible names/tooltips, focus rectangles are visible, high-contrast mode remains readable, and a fresh screenshot is captured.
  Complexity: M

- [ ] P2 - Add network-effect validation without default broad host blocking
  Why: Users want proof of reduced telemetry, but broad hosts/IP blocking has documented breakage risks.
  Evidence: Existing roadmap Pi-hole/NextDNS and pktmon ideas; W4RH4WK `block-telemetry.ps1` caveats; ChrisTitusTech/winutil issues 4710 and 4360.
  Touches: `TelemetrySlayer.ps1`, `README.md`
  Acceptance: Optional validation mode records before/after DNS or `pktmon` evidence for selected Microsoft telemetry endpoints and labels risky blocklist exports as opt-in.
  Complexity: L

- [ ] P2 - Add policy provenance metadata to every toggle
  Why: Users and admins need to see the source, supported builds, risk, and undo behavior behind each privacy setting without inspecting the script.
  Evidence: `TelemetrySlayer.ps1:119`, `TelemetrySlayer.ps1:438`, `TelemetrySlayer.ps1:701`; O&O ShutUp10++ recommendation tiers; Microsoft Learn policy docs.
  Touches: `TelemetrySlayer.ps1`, `README.md`, `tests/TelemetrySlayer.Tests.ps1`
  Acceptance: Each action has stable ID, source URL, policy path, supported OS/SKU/app gates, risk level, and undo type; GUI tooltips/details and README policy tables derive from this metadata.
  Complexity: M

- [ ] P2 - Complete Visual Studio CEIP policy coverage
  Why: The current Visual Studio toggle misses Microsoft's documented CEIP policy value under `VisualStudio\SQM`.
  Evidence: `TelemetrySlayer.ps1:277`, `TelemetrySlayer.ps1:1892`; Microsoft Visual Studio Experience Improvement Program docs.
  Touches: `TelemetrySlayer.ps1`, `README.md`, `tests/TelemetrySlayer.Tests.ps1`
  Acceptance: Scan/apply/undo include `HKLM:\Software\Policies\Microsoft\VisualStudio\SQM\OptIn=0`, existing feedback and `TurnOffSwitch` coverage remains, and tests verify the new registry target.
  Complexity: S

- [ ] P2 - Add restore history browsing and retention controls
  Why: Exact Undo restores only the latest snapshot, while backups are timestamped and useful for older recovery points.
  Evidence: `TelemetrySlayer.ps1:1172`, `TelemetrySlayer.ps1:1984`; Raphire/Win11Debloat release 2026.06.24 timestamped backup improvements; O&O ShutUp10++ config export/import pattern.
  Touches: `TelemetrySlayer.ps1`, `README.md`
  Acceptance: GUI can open the backup folder, list backup timestamps/manifests, restore a selected snapshot, and prune old backups by count or age while preserving the latest good restore.
  Complexity: M

- [ ] P2 - Add a local Windows Sandbox or VM Apply/Undo smoke
  Why: Mocked Pester tests prove catalog shape, but no local test exercises the elevated GUI worker, Apply/Undo round trip, or SKU/build gates on a disposable host.
  Evidence: `tests/TelemetrySlayer.Tests.ps1`, `TelemetrySlayer.ps1:1093`; Raphire/Win11Debloat release 2026.06.24 runspace-error fixes.
  Touches: `tests/`, `TelemetrySlayer.ps1`, `README.md`
  Acceptance: A local-only smoke command runs parser checks, Pester, and optional disposable Apply/Undo validation on Windows Sandbox/VM when available, emitting ignored artifacts and never requiring GitHub Actions.
  Complexity: L

- [ ] P3 - Prepare localization boundaries without translating yet
  Why: Comparable tools support multiple languages, but TelemetrySlayer currently embeds all UI/log strings directly in XAML and worker code.
  Evidence: `TelemetrySlayer.ps1:416`, `TelemetrySlayer.ps1:1119`; Sophia Script multilingual docs; WPD and DoNotSpy11 language support.
  Touches: `TelemetrySlayer.ps1`, `tests/TelemetrySlayer.Tests.ps1`
  Acceptance: Action IDs stay invariant, UI/log strings are centralized in an ASCII-safe lookup table, tests verify logic never depends on display text, and no translated strings are required for this step.
  Complexity: M
